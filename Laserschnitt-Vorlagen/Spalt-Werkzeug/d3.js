d3 = function () {
    ...
    d3.behavior.zoom = function () {
        ...
        function touchstarted() {
            var target = this,
                ...
                t = d3.select(target).on(mousedown, null),
                dragRestore = d3_event_dragSuppress();
    ...
    d3.geo.bounds = function () {
        ...
        var bound = {
            ...
            polygonEnd: function () {
                ...
                else if (dlambdaSum > euler) phi1 = 90;
                else if (dlambdaSum < -euler) phi0 = -90;
        ...
        function linePoint(lambda, phi) {
            ...
            if (p0) {
                ...
                var dlambda = lambda - lambda_,
                    ...
                    antimeridian = Math.abs(dlambda) > 180;
        ...
        for (var i = 0, n = polygon.length; i < n; ++i) {
            ...
            while (true) {
                ...
                var lambda = point[0],
                    ...
                    antimeridian = Math.abs(dlambda) > pi,
                    k = sinphi0 * sinphi;
        ...
        return {
            ...
            point: function (lambda1, phi1) {
                var slambda1 = lambda1 > 0 ? pi : -pi,
                    dlambda = Math.abs(lambda1 - lambda0);
    ...
    function d3_geo_clipCircle(radius) {
        var cr = Math.cos(radius),
            smallRadius = cr > 0,
            point = [radius, 0],
            notHemisphere = Math.abs(cr) > euler,
        ...
        function intersect(a, b, two) {
            ...
            var deltalambda = lambda1 - lambda0,
                polar = Math.abs(deltalambda - pi) < euler,
                meridian = polar || deltalambda < euler;
    ...
    d3.geo.albersUsa = function () {
        ...
        albersUsa.translate = function (_) {
            ...
            alaskaPoint = alaska.translate([x - .307 * k, y + .201 * k]).clipExtent([
                [x - .425 * k + euler, y + .12 * k + euler],
                [x - .214 * k - euler, y + .234 * k - euler]
            ]).stream(pointStream).point;
            hawaiiPoint = hawaii.translate([x - .205 * k, y + .212 * k]).clipExtent([
                [x - .214 * k + euler, y + .166 * k + euler],
                [x - .115 * k - euler, y + .234 * k - euler]
            ]).stream(pointStream).point;
            return albersUsa;
        };
        ...
        function resampleLineTo(x0, y0, lambda0, a0, b0, c0, x1, y1, lambda1, a1, b1, c1, depth, stream) {
            ...
            if (d2 > 4 * delta2 && depth--) {
                var a = a0 + a1,
                    ...
                    phi2 = Math.asin(c /= m),
                    lambda2 = Math.abs(Math.abs(c) - 1) < euler ? (lambda0 + lambda1) / 2 : Math.atan2(b, a),
    ...
    function d3_geo_lengthLineStart() {
        ...
        function nextPoint(lambda, phi) {
            var sinphi = Math.sin(phi *= d3_radians),
                cosphi = Math.cos(phi),
                t = Math.abs((lambda *= d3_radians) - lambda0),
    ...
    function d3_ease_elastic(a, p) {
        ...
        if (arguments.length) s = p / (2 * pi) * Math.asin(1 / a);
        else a = 1, s = p / 4;
        return function (t) {
    ...
    var d3_svg_arcOffset = -pi / 2,
        d3_svg_arcMax = 2 * pi - 1e-6;
